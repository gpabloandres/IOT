...
#define PUMP_PIN 12 // Pin del humificador
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == "control/pump") {
    if (message == "on") {
      digitalWrite(PUMP_PIN, HIGH);  // Encender humificador
      Serial.println("Humificador encendido");
    } else if (message == "off") {
      digitalWrite(PUMP_PIN, LOW);  // Apagar humificador
      Serial.println("Humificador apagado");
    }
  }

  // Controlar los coolers
  if (String(topic) == "control/coolers") {
    if (message == "on") {
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect("ESP8266Client", MQTT_username, MQTT_password)) {
      ...
      client.subscribe("control/pump"); // Suscribirse al tema para el control del humificador
      client.subscribe("control/coolers"); // Suscribirse al tema para el control de coolers
    } else {
...
void loop() {
  ...
  if (currentMillis - previousMillis > 10000) {
    ...
    // Controlar el humificador según la humedad
    if (humidity < 21) {
      digitalWrite(PUMP_PIN, HIGH);  // Encender humificador
      Serial.println("Humificador encendido");
    } else {
